...
pipeline {
    ...
    environment {
        ...
        MONGO_URI_SECRET = credentials("uidd-mongo-uri-production-base64");

        JWT_SECRET = credentials("uidd-secret-production-jwt");
        MAILCHIMP_KEY = credentials("uidd-secret-production-mailchimp-key");
        UPLOADER_AUTH = credentials("uidd-secret-production-uploader-auth");
        GOOGLE_SECRET = credentials("uidd-secret-production-google-secret");
        GOOGLE_CLIENT_ID = credentials("uidd-secret-production-google-client-id");
        FACEBOOK_SECRET = credentials("uidd-secret-production-facebook-secret");
        FACEBOOK_APP_ID = credentials("uidd-secret-production-facebook-app-id");
        TWITTER_SECRET = credentials("uidd-secret-production-twitter-secret");
        TWITTER_KEY = credentials("uidd-secret-production-twitter-key");
        MAILGUN_KEY = credentials("uidd-secret-production-mailgun-key");
        MAILGUN_DOMAIN = credentials("uidd-secret-production-mailgun-domain");
        ADMIN_FNAME = credentials("uidd-secret-production-admin-first-name");
        ADMIN_LNAME = credentials("uidd-secret-production-admin-last-name");
        ADMIN_EMAIL = credentials("uidd-secret-production-admin-email");
        ADMIN_PASSWORD = credentials("uidd-secret-production-admin-password");
        SLACK_TOKEN = credentials("uidd-secret-production-slack-token");
        APM_TOKEN = credentials("uidd-secret-production-apm-token");
        APM_URL = credentials("uidd-secret-production-apm-url");
        ELASTICSEARCH_URI = credentials("uidd-secret-production-elasticsearch-uri");
        ELASTICSEARCH_USER = credentials("uidd-secret-production-elasticsearch-user");
        ELASTICSEARCH_PASSWORD = credentials("uidd-secret-production-elasticsearch-password");
    ...
    stages {
        ...
        stage("Configure Secrets") {
            steps {
                ...
                sh "./jenkins/configure-secrets.sh $DOCKER_REGISTRY_SECRET eks/production/secrets/1-docker-registry-secret.yml"
                sh "./jenkins/configure-secrets.sh $MONGO_URI_SECRET eks/production/secrets/2-mongo-uri-secret.yml"
            }
        }
...
def cluster_setup_secrets() {
    withCredentials([
            ...
            stage("Deploy Secrets") {
                sshPut remote: remote, from: "eks/production/secrets", into: "."
